...
and games on the main one. Run while the rig server is stopped (it rewrites
the rig's config); rigrestart.ps1 does. With one monitor there is nothing to
move - the tests are headless then.

    python rigpos.py [rig folder]      (default: <repo>/.rig/testrig)
"""
import ctypes, ctypes.wintypes as w, json, os, sys
HERE = os.path.dirname(os.path.abspath(__file__))
RIG = sys.argv[1] if len(sys.argv) > 1 else os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(HERE))), ".rig", "testrig")
...
if not second:
    print("only one monitor - nothing to move")
    sys.exit(0)
_, L, T, R, B, dev = second[0]
path = os.path.join(RIG, "config.json")
...
def _rig_off_main(x, y):
    ...
    return x, y
'''
ov = os.path.join(RIG, "overlay.py")
