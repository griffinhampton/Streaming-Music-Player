...
with Windows Graphics Capture, encoded by the graphics card through Media
Foundation, pushed to `live.LiveEngine` as FLV video - all in this process,
no pixel ever on the CPU. Audio still comes from the page (AAC over the
WebSocket); the engine stamps both against one clock.
"""
...
class NativeVideo:
    def __init__(self, engine, log=None):
        ...
        self.error = ""
        self.stats = {"fps": 0, "dropped": 0, "size": "", "encoder": "", "frames": 0}
    ...
    def _loop(self, cap, enc, fps):
        period = 1.0 / fps
        t0 = time.perf_counter()
        next_tick = t0
        pending, tick, sent_avcc = False, 0, None
        rate_n, rate_t = 0, t0
            ...
            if now >= next_tick:
                ...
                if cap.poll() or cap.texture is not None:
                    if cap.texture_size != (enc.width, enc.height):
                        raise OSError(f"the window changed size to {cap.texture_size}")
                    pending = True
                    tick += 1
