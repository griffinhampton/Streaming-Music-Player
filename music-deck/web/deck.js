...
function fontOptions(inherit) {
  return fontOptionsFor(USER_FONTS, inherit);
}
...
async function addFonts(files, target) {
  ...
  if (!res.family) return false;
  FONT_FILES = res.fonts || FONT_FILES;
  USER_FONTS = familiesOf(FONT_FILES);
  fillFonts();
  if (target) {
    target.value = res.family;
    target.dispatchEvent(new Event('change', { bubbles: true }));
  }
  toast(`Added “${res.family}” — it's in every Font menu now`);
  return true;
}
...
/* (readControl, writeControl, showOut and the control markup are shared with
   the Canvas Builder: web/designer.js.)
